...
const SLICE      = 360 / TOTAL;  // 18 degrees per segment
const STAR_INDEX = 1;
const START_DEG  = -(STAR_INDEX * SLICE + SLICE / 2);

// ── Snap a rotation to the exact CENTER of its nearest segment ────────────────
// Without this, the wheel can stop visually between two segments.
function snapToSegmentCenter(rotDeg: number): number {
    // Round to nearest segment index then put it dead center
    return Math.round(rotDeg / SLICE) * SLICE;
}

// ── Calculate final rotation landing exactly at segment center ────────────────
function calcFinalRotation(currentRot: number, targetIndex: number): number {
    const fullSpins = (Math.floor(Math.random() * 5) + 7) * 360;

    // Exact center of target segment in wheel space
    ...
    // Ensure we never stop within the last 45° of a spin (prevents near-edge landings)
    if (delta < SLICE * 2) delta += 360;

    return currentRot + fullSpins + delta;
}

// ── Determine which segment is at the pointer after wheel stops ───────────────
function segmentAtTop(rotDeg: number): number {
    // Use the snapped value so this always agrees with what's drawn
    const snapped = snapToSegmentCenter(rotDeg);
...
const SpinWheel: FC<SpinProps> = ({ currentScore, onResult }) => {
    ...
    const spin = () => {
        ...
        // Snap to exact segment center — prevents visual "in-between" landing
        const snappedFinal = snapToSegmentCenter(finalRotDeg);
        ...
        // Loop spin sound continuously while wheel turns
        play("spin", 1, true);
        ...
        const animate = (now: number) => {
            ...
            if (p < 1) {
                ...
                // Draw the final snapped position before declaring result
                rotRef.current = snappedFinal;
                angleRef.current = snappedFinal * Math.PI / 180;
                drawFrame(angleRef.current);

                const actualIdx = segmentAtTop(snappedFinal);
                const seg = SEGMENTS[actualIdx];

                // Stop looping spin sound exactly when wheel stops
                stop("spin");
