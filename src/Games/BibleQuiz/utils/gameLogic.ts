...
export class BibleQuizGame {
    ...
    submitAnswer(question: BibleQuestion, answerIndex: number): AnswerResult {
        // Don't process answers if game is already over
        if (this.isGameOver) {
            return {
                correct: false,
                pointsEarned: 0,
                pointsDeducted: 0,
                pointsBefore: this.player.points,
                pointsAfter: this.player.points,
                pointsBreakdown: {
                    baseDeduction: 0,
                    levelPenalty: 0,
                    total: 0
        ...
        // Store points before any changes
        ...
        if (isCorrect) {
            ...
            // Streak bonus
            if (this.player.currentStreak >= 3) {
                streakBonus = Math.floor(this.player.currentStreak / 3) * 5;
            }

            // Level multiplier
            const levelMultiplier = 1 + ((this.currentLevel - 1) * 0.5);
            pointsEarned = Math.floor((basePoints + streakBonus) * levelMultiplier);

            // Update scores
            this.player.score += pointsEarned;
            this.player.points = Math.min(this.maxPoints, this.player.points + pointsEarned);
            ...
            if (this.correctInCurrentLevel >= this.questionsNeededToLevelUp && this.currentLevel < 3) {
                ...
                // Level up bonus
                const levelUpBonus = this.currentLevel * 50;
                this.player.score += levelUpBonus;
                pointsEarned += levelUpBonus;
                this.player.points = Math.min(this.maxPoints, this.player.points + levelUpBonus);
            ...
            // Check for score-based level
            const scoreBasedLevel = Math.floor(this.player.score / 200) + 1;
            if (scoreBasedLevel > this.player.level) {
                this.player.level = scoreBasedLevel;
            ...
            // Deduct points
            const baseDeduction = Math.floor(question.points * 0.5);
            const levelPenalty = (this.currentLevel - 1) * 5;
            pointsDeducted = Math.floor(baseDeduction + levelPenalty);

            // Apply deduction
            this.player.points = Math.max(0, this.player.points - pointsDeducted);

            // Penalty for wrong answer in higher levels
            if (this.currentLevel > 1 && this.correctInCurrentLevel > 0) {
                this.correctInCurrentLevel--;
            }
            ...
            // Check for game over - set both flags
            isGameOver = this.player.points <= 0;
            if (isGameOver) {
                this.isGameOver = true; // Set internal flag
            }
